const products = {
    pizza: [
        {
            name: 'Christmas Pizza',
            ingredients: 'Kalakutiena, spanguolių padažas, bruknės, šoninė, raudonieji svogūnai, mocarelos sūris, česnakinis padažas',
            price: 8.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Carbonara',
            ingredients: 'Sūrio padažas, granuliuoti česnakai, mocarelos sūris, raudonieji svogūnai, šoninė, vyšniniai pomidorai, itališkos žolelės, parmezanas, čederio sūris',
            price: 8.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Cheesy Chicken',
            ingredients: 'Pomidorai, mocarelos sūris, vištiena, sūrio padažas',
            price: 7.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Cheese',
            ingredients: 'Mocarelos sūris, picų padažas, fetos suгis, itališkos žolelės',
            price: 6.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'The Meats',
            ingredients: 'Medžiotojų dešrelės, mocarelos sūris, šoninė, kumpis, jautiena, picų padažas',
            price: 7.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Christmas Pizza',
            ingredients: 'Kalakutiena, spanguolių padažas, bruknės, šoninė, raudonieji svogūnai, mocarelos sūris, česnakinis padažas',
            price: 8.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Carbonara',
            ingredients: 'Sūrio padažas, granuliuoti česnakai, mocarelos sūris, raudonieji svogūnai, šoninė, vyšniniai pomidorai, itališkos žolelės, parmezanas, čederio sūris',
            price: 8.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Cheesy Chicken',
            ingredients: 'Pomidorai, mocarelos sūris, vištiena, sūrio padažas',
            price: 7.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'Cheese',
            ingredients: 'Mocarelos sūris, picų padažas, fetos suгis, itališkos žolelės',
            price: 6.75,
            photo: './img/christmas.jpg'
        },
        {
            name: 'The Meats',
            ingredients: 'Medžiotojų dešrelės, mocarelos sūris, šoninė, kumpis, jautiena, picų padažas',
            price: 7.75,
            photo: './img/christmas.jpg'
        },

    ],
    snacks: [
        {
            name: 'Brusketa su pomidorais, 25 cm',
            ingredients: 'Brusketa su pomidorais, česnako padažo ir mocarela.',
            price: 5.50,
            photo: '#'
        }, {
            name: 'Cezario salotos',
            ingredients: 'Vištiena, vynuoginiai pomidorai, gūžinės salotos Iceberg, skrebučiai, parmezano sūris, bazilikai ir cezario padažas',
            price: 3.50,
            photo: '#'
        }, {
            name: 'Dodsteris',
            ingredients: 'Dodsteriai yra unikalus produktas, mūsų išradimas. Atraskite dodsterius. Tai kai kas naujo!',
            price: 2.80,
            photo: '#'
        }, {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        }
    ],
    desserts: [
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        },
        {
            name: 'Vištienos kepsneliai, 7 vnt.',
            ingredients: 'Krosnyje kepti vištienos gabaliukai',
            price: 2.75,
            photo: '#'
        }

    ],
    drinks: [
        {
            name: 'Coca-Cola',
            size: '1 l',
            price: 2.50,
            photo: '#'
        },
        {
            name: 'Coca-Cola',
            size: '0.5 l',
            price: 1.50,
            photo: '#'
        },
        {
            name: 'Coca-Cola',
            size: '1 l',
            price: 2.50,
            photo: '#'
        },
        {
            name: 'Coca-Cola',
            size: '0.5 l',
            price: 1.50,
            photo: '#'
        },
        {
            name: 'Coca-Cola',
            size: '1 l',
            price: 2.50,
            photo: '#'
        },
        {
            name: 'Coca-Cola',
            size: '0.5 l',
            price: 1.50,
            photo: '#'
        }
    ],
    otherProducts: [
        {
            name: 'DODO kuponas didelei picai',
            description: 'DODO kuponas didelei picai. Galioja pristatant į namus ir picerĳoje',
            price: 12.95,
            photo: '#'
        },

    ]



};

export { products };